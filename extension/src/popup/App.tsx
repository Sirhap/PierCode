...
function Toggle({
  ...
  return (
    <div className="flex items-start justify-between gap-3">
      ...
      >
        <span className={`inline-block w-5 h-5 mt-0.5 rounded-full shadow transition-transform duration-200 ${checked ? 'translate-x-5' : 'translate-x-0.5'}`} style={{ background: checked ? '#0e1116' : 'var(--dim)' }} />
      </button>
    </div>
  )
}
    ...
    <div className="w-72 p-4" style={{ background: 'var(--bg)', color: 'var(--txt)' }}>
      ...
      {toast && (
        <div
          ...
          style={{ background: toast.type === 'success' ? 'var(--glow)' : 'var(--red)', color: '#0e1116' }}
        >
          {toast.msg}
        </div>
      )}
